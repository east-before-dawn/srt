function [Texture] = texture_f(name)
...
%Í¼Æ¬´óÐ¡±ØÐë´óÓÚ4*4²ÅÄÜ¼ÆËãE1h¡¢E1v 
if (Nx<4||Ny<4) 
    flag=1; 
end 
if(flag==0) 
...
%Í¼Æ¬´óÐ¡±ØÐë´óÓÚ8*8²ÅÄÜ¼ÆËãE2h¡¢E2v 
if (Nx<8||Ny<8) 
    flag=1; 
end 
if(flag==0) 
...
%Í¼Æ¬´óÐ¡±ØÐë´óÓÚ16*16²ÅÄÜ¼ÆËãE3h¡¢E3v 
if (Nx<16||Ny<16) 
    flag=1 
end 
if(flag==0) 
...
%Í¼Æ¬´óÐ¡±ØÐë´óÓÚ32*32²ÅÄÜ¼ÆËãE4h¡¢E4v  
if (Nx<32||Ny<32) 
    flag=1; 
end 
if(flag==0) 
...
%Í¼Æ¬´óÐ¡±ØÐë´óÓÚ64*64²ÅÄÜ¼ÆËãE5h¡¢E5v 
if (Nx<64||Ny<64) 
    flag=1; 
end 
if(flag==0) 
...
ans1 = Fcoarseness+Fcontrast+Fdirection;
Fcoarseness = Fcoarseness / ans1;
Fcontrast = Fcontrast / ans1;
Fdirection = Fdirection / ans1;
Texture = [Fcoarseness, Fcontrast, Fdirection];
% disp('´Ö²Ú¶È£º');display(Fcoarseness) 
% disp('¶Ô±È¶È£º');display(Fcontrast) 
% disp('·½Ïò¶È£º');display(Fdirection) 
% deltaT=cputime-t0; 
% display(deltaT); 